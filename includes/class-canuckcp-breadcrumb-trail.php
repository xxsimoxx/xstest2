...
function canuckcp_custom_breadcrumbs() {
	...
	$breadcrumb_args = array(
		...
		'labels'        => array(
			...
			'home'                => '<i class="font-icon" title="' . esc_attr__( 'Home', 'canuck-cp' ) . '">' . canuckcp_svg( 'home', $icon_width = '14', $icon_color = '#7f7f7f' ) . '</i>',
	...
	if ( function_exists( 'canuckcp_breadcrumb_trail' ) ) {
		canuckcp_breadcrumb_trail( $breadcrumb_args );
	}
}
 ...
 * @return string of html breadcrumbs.
 */
function canuckcp_breadcrumb_trail( $args = array() ) {
	$breadcrumb = apply_filters( 'breadcrumb_trail_object', null, $args );
	if ( ! is_object( $breadcrumb ) ) {
		$breadcrumb = new CanuckCP_Breadcrumb_Trail( $args );
	}
 ...
 * @return void
 */
function canuckcp_breadcrumb_trail_print_styles() {
	$style = '
		.breadcrumbs .trail-browse,.breadcrumbs .trail-items,
		.breadcrumbs .trail-items li                          { display: inline-block; margin: 0; padding: 0; border: none; background: transparent; text-indent: 0; }
...
add_action( 'wp_head', 'canuckcp_breadcrumb_trail_print_styles' );
 ...
 * @access public
 */
class CanuckCP_Breadcrumb_Trail {
	 ...
	 * @since  0.6.0
	 * @access public
	 * @param  array $args is the array of arguments for the class.
	 *
	...
	protected function set_labels() {
		$defaults = array(
			'browse'              => esc_html__( 'Browse:', 'canuck-cp' ),
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'canuck-cp' ),
			'home'                => esc_html__( 'Home', 'canuck-cp' ),
			'error_404'           => esc_html__( '404 Not Found', 'canuck-cp' ),
			'archives'            => esc_html__( 'Archives', 'canuck-cp' ),
			...
			'paged'               => esc_html__( 'Page %s', 'canuck-cp' ),
			// Translators: %s is the page number.
			'paged_comments'      => esc_html__( 'Comment Page %s', 'canuck-cp' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s', 'canuck-cp' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s', 'canuck-cp' ),
	...
	protected function add_singular_items() {
		...
		if ( $post_title = single_post_title( '', false ) ) {// phpcs:ignore
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'canuck-cp' ) ) );
		}
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'canuck-cp' ) ) );
		}
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'canuck-cp' ) ) );
		}
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'canuck-cp' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'canuck-cp' ) ) );
		$day   = sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'canuck-cp' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'canuck-cp' ) ) );
		$week = sprintf( $this->labels['archive_week'], get_the_time( esc_html_x( 'W', 'weekly archives date format', 'canuck-cp' ) ) );
		// Add the year item.
		$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y' ) ) ), $year );
		// Add the week item.
		if ( is_paged() ) {
	 ...
	 * @since  1.0.0
	 * @access protected
	 * @param  int $post_id is the ID number of the post.
	 * @return void
	 */
	protected function add_post_parents( $post_id ) {
		...
		while ( $post_id ) {
			...
			if ( 'page' == $post->post_type && 'page' == get_option( 'show_on_front' ) && $post_id == get_option( 'page_on_front' ) ) {// phpcs:ignore
				break;
			}
	 ...
	 * @since  1.0.0
	 * @access protected
	 * @param  int $post_id is the ID of the post.
	 * @return void
	 */
	...
	protected function add_path_parents( $path ) {
		...
		if ( ! empty( $post ) ) {
			...
			$path = trim( $path, '/' );
			preg_match_all( "/\/.*?\z/", $path, $matches );// phpcs:ignore
			// If matches are found for the path.
			if ( isset( $matches ) ) {
				// Reverse the array of matches to search for posts in the proper order.
				$matches = array_reverse( $matches );
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag ) {// phpcs:ignore
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'canuck-cp' ) ) ) );
				} elseif ( '%monthnum%' == $tag ) {// phpcs:ignore
					// If using the %monthnum% tag, add a link to the monthly archive.
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'canuck-cp' ) ) ) );
				} elseif ( '%day%' == $tag ) {// phpcs:ignore
					// If using the %day% tag, add a link to the daily archive.
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'canuck-cp' ) ) ) );
				} elseif ( '%author%' == $tag ) {// phpcs:ignore
